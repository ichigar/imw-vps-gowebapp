...
- Ejecuta procesos bajo un usuario sin privilegios y controla el arranque con `systemd`.
- Mantén logs separados por aplicación en `/var/log/<appname>/` y configura `logrotate`.
- Configura `ufw` y abre solo los puertos necesarios.
- Revisa y prueba scripts localmente antes de ejecutarlos en producción.

## Añadir este documento al repositorio local y remoto

```bash
git add docs/production_setup.md
git commit -m "docs: guía de despliegue y proxy para VPS Ubuntu"
git push origin main
```

(Ajusta la rama si usas `master` u otra.)

## Opciones que puedo preparar para ti

- Crear la unidad `systemd` adaptada a una app específica dentro de `apps/` (indícame ruta y nombre del jar/bin).
- Generar el `Caddyfile` y el script `setup_caddy_reverse_proxy` a partir de plantillas.
- Preparar un PR con este archivo (necesitaré que hagas el push localmente o me proporciones acceso para crear el PR).
